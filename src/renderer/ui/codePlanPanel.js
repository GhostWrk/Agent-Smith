...
(function (global) {
    ...
    let mountEl = null;
    let state = { sessionId: null, codePlan: null, goal: '', phase: 'idle' };
    ...
    function progressLabel(codePlan) {
    ...
    function renderExecuting({ goal, codePlan, takeover = false }) {
        if (!mountEl) return;
        state = { ...state, codePlan, goal, phase: 'executing' };
        const prog = progressLabel(codePlan);
        const cur = (codePlan?.steps || []).find(s => s.status === 'active');
        mountEl.innerHTML = `
            <div class="agent-run-card arc-executing">
                ...
                <div class="arc-body">
                    <div class="arc-current"><strong>Now:</strong> ${escapeHtml(cur?.title || 'Working…')}</div>
    ...
    function updateProgress(codePlan) {
        if (!codePlan) return;
        state.codePlan = codePlan;
        if (state.phase === 'executing') {
            renderExecuting({ goal: state.goal, codePlan, takeover: false });
        } else if (window.XKSidebarLayout?.updatePlanChip) {
            window.XKSidebarLayout.updatePlanChip(progressLabel(codePlan));
        }
    }

    function clear() {
        state = { sessionId: null, codePlan: null, goal: '', phase: 'idle' };
